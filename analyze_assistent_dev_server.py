...
def main():
    ...
    for exp in exps:
        ...
        ts = client.search_traces(experiment_ids=[experiment_id], filter_string="trace.name = 'domino_eval_trace'")

        # group traces by input
        trace_df = pd.DataFrame({'inputs': [], 'outputs': [], 'url': [], 'evaluation_result':[] })
        for t in ts:
